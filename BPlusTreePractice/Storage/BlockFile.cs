...
namespace BPlusTreePractice
{
  ...
  internal class BlockFile
  {
    ...
    internal int FileHeaderSize
    {
      get
      {
        ...
        return FileHeaderPrefix.Length      // 魔数前缀长度
          + StorageConstants.IntegerLength; // 块的大小 BlockSize
      }
    }
    ...
    public BlockFile(Stream stream, long seekStart, byte[] blockFileHeaderPrefix, int blockSize)
    {
      ...
      if (this.FileHeaderPrefix == null || this.FileHeaderPrefix.Length < StorageConstants.MinFileHeaderPrefixLength)
      {
        throw new BlockFileException("Block file header prefix length is too small.");
      }
      if (this.BlockSize < StorageConstants.MinBlockSize)
      {
    ...
    private void ReadFileHeader()
    {
      ...
      // 块的大小 4 Bytes
      this.BlockSize = StorageHelper.RetrieveInt(header, index);
    ...
    public byte[] MakeFileHeader()
    {
      ...
      // 块的大小 4 Bytes
      StorageHelper.Store(this.BlockSize, header, FileHeaderPrefix.Length);
